...
Notes for next time:
 - Similar to attempt 1, just re-implementing and switching to a generator
"""
...
def tree_to_leaves(root: Optional[TreeNode]):
    ...
    while len(d) > 0:
        ...
        if e.right is not None:
            d.appendleft(e.right)
        if e.left is not None:
            d.appendleft(e.left)
        
        if e.right is None and e.left is None:
            yield e.val

class Solution:
    def leafSimilar(self, root1: Optional[TreeNode], root2: Optional[TreeNode]) -> bool:
        g1 = tree_to_leaves(root1)
        g2 = tree_to_leaves(root2)
        for e1 in g1:
            e2 = next(g2, None)
            if e1 != e2:
                return False
